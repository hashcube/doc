...
the Hashcube DevKit. The [source code](https://github.com/Hashcube/whack-that-mole)
is available and in this guide we'll step through it
to see how the components of the Hashcube DevKit fit together.

This walk-through assumes that you have the Hashcube DevKit
...
the developer environment of the Hashcube DevKit so that
...
A game created with the Hashcube DevKit has this basic file structure:
...
First, the Hashcube DevKit is initialized and the
...
Once the Hashcube DevKit environment is initialized, the
...
For examples of using ViewPools, see the official [ViewPools example](http://doc.Hashcube.com/example/views-pools/) or the Isometric game engine's [GridLayerView](https://github.com/Hashcube/isometric/blob/master/views/GridLayerView.js).
...
A good parent View to use is the GestureView.  See the [GestureView example](http://doc.Hashcube.com/example/events-input-gesture/) for how to subscribe to Swipe, Pinch, Rotate, DragSingle, FingerDown, FingerUp, and ClearMulti events.
